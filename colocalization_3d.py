...
def create_dimetric_thumbnail(crop_r, crop_g):
    """
    Creates a 3D Volume Rendering with "Glowing" Red Channel.
    - Normalizes data to 0-100 scale to fix contrast issues.
    - Saturates Red Color early so medium-intensity pixels are BRIGHT.
    - Hides Green background noise.
    """
    # --- 1. PRE-PROCESSING ---
    s_start, s_end = 4, 17
    sub_r = crop_r[s_start:s_end, s_start:s_end, s_start:s_end].copy()
    sub_g = crop_g[s_start:s_end, s_start:s_end, s_start:s_end].copy()
    ...
    # --- 3. HELPER: Create Volume ---
    def create_vol_actor(data_arr, color_rgb, is_red=False):
        # Safety check
        if data_arr.size == 0: return None, None
        
        d_min = data_arr.min()
        d_max = data_arr.max()
        
        # If flat data (e.g. all zeros), skip
        if d_max <= d_min: return None, None

        # --- INTERNAL NORMALIZATION (Crucial Step) ---
        # Scale everything to 0.0 -> 100.0
        # This fixes "invisible" signals by standardizing the range.
        scale_factor = 100.0 / (d_max - d_min)
        data_flat = (data_arr.ravel() - d_min) * scale_factor
        
        vtk_data = numpy_support.numpy_to_vtk(num_array=data_flat, deep=True, array_type=vtk.VTK_FLOAT)
        ...
        if is_red:
            # --- RED CHANNEL STRATEGY: GLOWING CORE ---
            
            # Opacity: Steep Ramp to make it SOLID
            opacity_func.AddPoint(0, 0.0)
            opacity_func.AddPoint(15, 0.0)   # Hide bottom 15% noise
            opacity_func.AddPoint(30, 0.8)   # Rapidly jump to 80% opacity
            opacity_func.AddPoint(100, 1.0)  # Max solid
            
            # Color: SATURATE EARLY (Fixes Dark Red issue)
            # Instead of reaching full Red at 100, we reach it at 40.
            # This makes "medium" pixels look bright red, not dark maroon.
            color_func.AddRGBPoint(0, 0, 0, 0)
            color_func.AddRGBPoint(15, 0, 0, 0)
            color_func.AddRGBPoint(40, *color_rgb) # Full Brightness at 40% intensity!
            color_func.AddRGBPoint(100, *color_rgb)
            
            # Lighting: High Ambient for "Self-Luminous" look
            vol_prop.SetAmbient(0.8)  # 80% Ambient light (Glow)
            vol_prop.SetDiffuse(0.3)
            vol_prop.SetSpecular(0.2)
            
        else:
            # --- GREEN CHANNEL STRATEGY: FAINT CONTEXT ---
            
            # If Green range is tiny (just noise, e.g. 1.0 to 1.2), keep it very faint
            # Raw range check
            raw_range = d_max - d_min
            max_opacity = 0.2 if raw_range > 5 else 0.05
            
            opacity_func.AddPoint(0, 0.0)
            opacity_func.AddPoint(40, 0.0)   # Hide bottom 40% (removes fog)
            opacity_func.AddPoint(100, max_opacity) 
    ...
    # --- 4. ADD VOLUMES ---
    vol_r, img_ref = create_vol_actor(sub_r, (1.0, 0.2, 0.2), is_red=True) # Slightly light red for better visibility
    if vol_r: renderer.AddVolume(vol_r)
    
    vol_g, _ = create_vol_actor(sub_g, (0.0, 1.0, 0.0), is_red=False)
    if vol_g: renderer.AddVolume(vol_g)
    
    # --- 5. ADD THIN WHITE BORDER ---
    if img_ref:
        outline = vtk.vtkOutlineFilter()
        outline.SetInputData(img_ref)
        ...
        actor_outline.GetProperty().SetColor(1.0, 1.0, 1.0)
    ...
    del vol_r, vol_g, renderer, render_window, writer, w2if
...
if __name__ == "__main__":
    ...
    if os.path.exists(base_path):
        main(
            ...
            os.path.join(base_path, "Cell2_NoGlucose60min_3D_Report_VTK.pptx")
        )
    else:
